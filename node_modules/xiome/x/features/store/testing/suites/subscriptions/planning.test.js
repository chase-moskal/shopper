...
export default suite({
    "a user with clerk permisisons": {
        async "can create a new subscription plan"() {
            ...
            function expectPlanAndTierIsLocallyPresent() {
                ...
                expect(plans[0].tiers[0].pricing[0].price).equals(1000);
            }
        ...
        async "can create multiple plans, and the tiers aren't scrambled"() {
            ...
            const getPlan = (id) => getPlans().find(p => p.planId === id);
        ...
        async "can add a new tier to an existing plan"() {
            ...
            expect(tier).ok();
            expect(tier.pricing[0].price).equals(10000);
            {
                const { store } = await api
                    .client(api.roles.clerk)
                    .then(x => x.browserTab());
                ...
                expect(anotherTier.pricing[0].price).equals(10000);
            }
        },
        ...
        async "can edit a tier"() {
            ...
            expect(tier1.pricing[0].price).equals(500);
            function pricingToDraft(pricing) {
                return {
                    price: pricing.price,
    ...
    "a user with regular permissions": {
        async "cannot create a new subscription plan"() {
            const customer = await connectedStore()
