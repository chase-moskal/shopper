...
import { nap, badOrigin, makeHostOptions, makeBridgedSetup, makeClientOptions, } from "./testing.js";
export default {
    "client/host integration": {
        "wakeup call from host is received by client": async () => {
            ...
            const mock = hostPostMessage.mock;
            await nap();
            assert(mock.calls.length > 0, `host postMessage must be called`);
            assert(mock.calls[0].args[0].signal === 1 /* Signal.Wakeup */, `first call to host postMessage must be wakeup signal`);
            return true;
        },
        ...
        "end to end call requests": async () => {
            ...
            return (expect(result1).equals(3) &&
        ...
        "client can listen and unlisten to host events": async () => {
            ...
            let result1 = false;
            let result2 = false;
            const listener = event => { result1 = event.alpha; };
    ...
    "host": {
        ...
        "sends a wakeup mesesage": async () => {
            const options = makeHostOptions();
            crosscallHost(options);
            const mock = options.shims.postMessage.mock;
            assert(mock.calls.length > 0, `host postMessage wasn't even called`);
            const [message, origin] = mock.calls[0].args;
            return (expect(message.id).equals(0) &&
                expect(message.signal).equals(1 /* Signal.Wakeup */) &&
                expect(origin).equals("*"));
        },
        "binds message event listener": async () => {
            const options = makeHostOptions();
            crosscallHost(options);
            const mock = options.shims.addEventListener.mock;
        ...
        "unbinds message event listener on deconstructor": async () => {
            ...
            host.stop();
            const mock = options.shims.removeEventListener.mock;
            return expect(mock.calls.length).equals(1);
        },
    },
    "client": {
